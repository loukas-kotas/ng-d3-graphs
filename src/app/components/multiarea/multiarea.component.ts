...
export class MultiareaComponent implements OnInit {
  ...
  private render() {
    ...
    // SCALE
    xScale.domain(d3.extent(this.labels, (d) => {
      return d;
    }));
    ...
    const xAxis = this.d3Service.getXaxisTime(
        svg, height, xScale, this.options.timeFormat, this.options.xAxisTicks);
    ...
    if (this.options.legend) {
      this.addLegend(
          svg, (width - (this.options.margin.left * 4)), 10, colorScale,
          this.labelsAndData.map(d => d.label), this.options.legendFontSize,
          this.options.legendWidth, this.options.legendHeight,
          this.options.legendBackground);
    }
  }
